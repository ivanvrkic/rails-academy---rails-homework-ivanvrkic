...
class Flight < ApplicationRecord
  ...
  def not_overlapping
    ...
    errors.add(:departs_at, "from #{self.id} #{self.departs_at} #{self.arrives_at} must not overlap with existing flights #{is_overlapping.id} #{is_overlapping.departs_at} #{is_overlapping.arrives_at}within the same company")
